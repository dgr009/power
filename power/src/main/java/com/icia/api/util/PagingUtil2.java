...
public class PagingUtil2 {
	...
	private static int numberOfArticleForPage = 9; // 한페이지당 출력할 글의 개수
	...
	public static Pagination setPageMaker(int pageNo, int numberOfTotalArticle) {
		...
		if (pageNo > numberOfTotalPage)
			pageNo = numberOfTotalPage;
		...
		if (endPage >= numberOfTotalPage)
			endPage = numberOfTotalPage;
		else
			endPage = startPage + sizeOfPagination - 1;
